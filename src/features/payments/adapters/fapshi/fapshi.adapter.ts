/**
 * Implémentation Fapshi Direct Pay de IPaymentProvider.
 *
 * Mode DIRECT : envoie une notification de paiement sur le mobile (POST /direct-pay).
 *
 * Ce que cet adapter fait : communiquer avec l'API Fapshi, transformer les types,
 * vérifier les signatures webhook, traduire les erreurs.
 * Ce qu'il ne fait JAMAIS : accès DB, règles métier, commissions, notifications.
 */
...
export interface FapshiAdapterConfig {
...
export class FapshiAdapter implements IPaymentProvider {
  ...
  constructor(config: FapshiAdapterConfig) {
