          ...
          <table className="w-full">
            ...
            <tbody>
              {recentTransactions.map((trx) => (
                <tr key={trx.id} className="border-b border-outline-variant/50 hover:bg-surface-container-low transition-colors">
                  <td className="px-6 py-4 font-body-md text-primary font-mono text-sm">{trx.id.substring(0, 8)}...</td>
                  <td className="px-6 py-4 font-body-sm text-on-surface-variant">{trx.order?.number ?? "N/A"}</td>
                  <td className="px-6 py-4 font-body-md text-primary">${Number(trx.amount).toFixed(2)}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 font-label-caps text-[10px] uppercase ${trx.status === "SUCCESS" ? "bg-secondary-fixed text-on-secondary-fixed" : trx.status === "FAILED" ? "bg-error-container text-on-error-container" : "bg-surface-container text-on-surface-variant"}`}>
                      {trx.status === "SUCCESS" ? "Reussie" : trx.status === "FAILED" ? "Echouee" : trx.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-body-sm text-on-surface-variant">{new Date(trx.createdAt).toLocaleDateString("fr")}</td>
                </tr>
              ))}
            </tbody>
          </table>
