    ...
    if (res.ok) {
      setUsers((prev) =>
        prev.map((u) => {
          ...
          return {
            ...u,
            userRoles: u.userRoles.filter(
              (ur) => props.allRoles.find((r) => r.id === roleId)?.name !== ur.role.name
            ),
          };
        })
      );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
        <div>
          ...
          <p className="font-body-md text-body-md text-on-surface-variant max-w-2xl">
            Gerer les participants, assigner les roles et surveiller les comptes.
          </p>
        </div>
      </div>
          ...
          <tbody>
            {users.map((user) =>
              editingId === user.id ? (
                <tr
                  key={user.id}
                  className="border-b border-outline-variant/50 bg-surface-container-low"
                >
                  <td colSpan={5} className="px-6 py-4">
                    <form
                      onSubmit={(e) => handleSaveEdit(e, user)}
                      className="grid grid-cols-1 md:grid-cols-4 gap-4"
                    >
                      <input
                        name="firstName"
                        defaultValue={user.firstName}
                        className="bg-transparent border-b border-outline-variant py-2 px-0 text-body-md text-primary outline-none focus:border-primary"
                        placeholder="Prenom"
                      />
                      <input
                        name="lastName"
                      ...
                      <div className="md:col-span-4 flex gap-3 justify-end mt-2">
                        <button
                          type="submit"
                          disabled={pending}
                          className="bg-primary text-on-primary font-label-caps text-label-caps uppercase py-2 px-4 hover:bg-inverse-surface transition-colors disabled:opacity-70"
                        >
                          {pending ? "..." : "Enregistrer"}
                        </button>
                        <button
                          ...
                          className="border border-primary text-primary font-label-caps text-label-caps uppercase py-2 px-4 hover:bg-surface-container transition-colors"
                        >
                          Annuler
                        </button>
                      </div>
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      {user.userRoles.map((ur) => (
                        <span
                          key={ur.role.name}
                          className="px-2 py-1 bg-surface-container text-on-surface-variant font-label-caps text-[10px] uppercase border border-outline-variant inline-flex items-center gap-1"
                        >
                          {ur.role.name}
                          <button
                            onClick={() =>
                              handleRoleChange(
                                user.id,
                                props.allRoles.find((r) => r.name === ur.role.name)?.id || "",
                                "remove"
                              )
                            }
                      ...
                      <select
                        ...
                        value=""
                        onChange={(e) => {
                          if (e.target.value) handleRoleChange(user.id, e.target.value, "add");
                          e.target.value = "";
                        }}
                      >
                        <option value="">+ Role</option>
                        {props.allRoles
                          .filter((r) => !user.userRoles.some((ur) => ur.role.name === r.name))
                          .map((r) => (
                  ...
                  <td className="px-6 py-4">
                    <button
                      ...
                      className={`px-2 py-1 font-label-caps text-[10px] uppercase hover:opacity-80 transition-opacity ${user.isActive ? "bg-secondary-container text-on-secondary-container" : "bg-error-container text-on-error-container"}`}
                    >
