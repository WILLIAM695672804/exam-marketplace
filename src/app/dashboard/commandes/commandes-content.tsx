    ...
    <div className="max-w-container-max mx-auto w-full flex flex-col gap-stack-lg">
      ...
      <div className="flex border-b border-outline-variant w-full">
        <button
          ...
          className={`py-4 px-6 font-label-caps text-label-caps uppercase transition-colors border-b-2 ${activeTab === "pending" ? "text-primary border-primary font-bold" : "text-on-surface-variant border-transparent hover:text-primary"}`}
        >
          En attente ({pendingOrders.length})
        </button>
        <button
          onClick={() => setActiveTab("completed")}
          className={`py-4 px-6 font-label-caps text-label-caps uppercase transition-colors border-b-2 ${activeTab === "completed" ? "text-primary border-primary font-bold" : "text-on-surface-variant border-transparent hover:text-primary"}`}
        >
      ...
      {loading ? (
        <div className="animate-pulse space-y-4">
          {[1, 2].map((i) => (
            <div key={i} className="h-24 bg-surface-variant" />
          ))}
        </div>
      ) : activeTab === "pending" ? (
        <div className="flex flex-col gap-6">
          {pendingOrders.length === 0 ? (
            ...
            pendingOrders.map((order) => (
              <div
                key={order.id}
                className="py-6 flex flex-col md:flex-row md:items-center justify-between border-b border-outline-variant gap-4"
              >
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-3">
                    <span className="font-body-lg text-body-lg text-primary">{order.number}</span>
                    <span className="px-2 py-1 bg-surface-container-high text-on-surface-variant font-label-caps text-[10px] uppercase">
                      En attente
                    </span>
                  </div>
              ...
              <div key={order.id} className="flex flex-col gap-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-primary pb-4">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="px-2 py-1 bg-surface-container-high text-on-surface font-label-caps text-[10px] uppercase border border-outline-variant">
                      Payee
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  {order.items.map((item) => (
                    <div
                      key={item.id}
                      className="bg-surface-bright border border-outline-variant p-6 flex flex-col md:flex-row justify-between gap-6 hover:border-primary transition-colors duration-300"
                    >
                      <div className="flex flex-col gap-3 max-w-xl">
                        <h4 className="font-headline-sm text-primary leading-tight">
                          {item.titleSnapshot}
                        </h4>
                        <div className="flex flex-wrap gap-2">
                          {item.withCorrection && (
                            <span className="px-2 py-1 bg-surface-container text-on-surface-variant font-label-caps text-[10px] uppercase">
                              Avec corrige
                            </span>
                          )}
                          <span className="px-2 py-1 bg-surface-container text-on-surface-variant font-label-caps text-[10px] uppercase">
                      ...
                      <div className="flex flex-col md:items-end justify-center gap-3 shrink-0">
                        <div className="flex gap-3">
                          <button
                            onClick={() => handleDownload(item.id, "paper")}
                            className="py-2 px-4 border border-primary text-primary font-label-caps text-label-caps uppercase hover:bg-surface-container-high transition-colors flex items-center gap-2"
                          >
                          ...
                          {item.withCorrection && (
                            <button
                              onClick={() => handleDownload(item.id, "correction")}
                              className="py-2 px-4 border border-primary text-primary font-label-caps text-label-caps uppercase hover:bg-surface-container-high transition-colors flex items-center gap-2"
                            >
                        ...
                        {item.review ? (
                          <div className="flex items-center gap-1 mt-2 text-secondary">
                            ...
                            <span className="font-label-caps text-[10px] uppercase ml-2 text-on-surface-variant">
                              Evalue
                            </span>
                          </div>
                        ) : (
                          <button className="text-on-surface-variant font-label-caps text-label-caps uppercase hover:text-primary transition-colors underline decoration-1 underline-offset-4 mt-2">
                            Laisser un avis
                          </button>
                        )}
                      </div>
