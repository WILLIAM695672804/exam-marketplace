...
export function CategoriesManager({ initialCategories }: { initialCategories: Category[] }) {
  ...
  async function handleEdit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (res.ok) {
      setCategories((prev) =>
        prev.map((c) =>
          c.id === editing.id ? { ...c, name, description: form.get("description") as string } : c
        )
      );
      resetForm();
    }
      ...
      <div className="flex justify-between items-end mb-12">
        <div>
          ...
          <p className="font-body-md text-on-surface-variant">
            Ajouter, modifier ou supprimer des categories.
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {(showAdd || editing) && (
        <div className="mb-12 bg-surface-container-lowest border border-outline-variant p-8">
          <h3 className="font-headline-sm text-primary mb-6">
            {editing ? "Modifier" : "Nouvelle"} categorie
          </h3>
          <form onSubmit={editing ? handleEdit : handleAdd} className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-transparent border-0 border-b border-outline-variant py-2 px-0 text-body-md text-primary focus:ring-0 focus:border-primary outline-none resize-none"
              />
            </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-outline-variant">
              ...
              <th className="text-right font-label-caps text-label-caps text-on-surface-variant uppercase px-6 py-4">
                Actions
              </th>
            </tr>
          </thead>
              ...
              >
                ...
                <td className="px-6 py-4 text-right">
                  ...
                  <button
                    ...
                    disabled={(cat._count?.competitions ?? 0) > 0}
                    title={
                      (cat._count?.competitions ?? 0) > 0 ? "Supprimez d'abord les concours" : ""
                    }
                  >
                    <span
                      className={`material-symbols-outlined text-[18px] ${(cat._count?.competitions ?? 0) > 0 ? "text-outline-variant" : "text-error"}`}
                    >
                      delete
                    </span>
                  </button>
