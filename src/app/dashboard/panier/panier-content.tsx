...
export function PanierContent() {
  ...
  async function handleToggleCorrection(examPaperId: string, current: boolean) {
    setItems((prev) =>
      prev.map((i) => (i.examPaper.id === examPaperId ? { ...i, withCorrection: !current } : i))
    );
    await fetch("/api/cart", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ examPaperId, withCorrection: !current }),
    });
  }
    ...
    <div className="max-w-container-max mx-auto w-full">
      <div className="mb-12">
        ...
        <p className="font-body-md text-body-md text-on-surface-variant">
          Gerer les epreuves dans votre panier avant de passer commande.
        </p>
      </div>
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-section-gap text-center border border-outline-variant bg-surface-container-lowest">
          ...
          <p className="font-body-lg text-body-lg text-on-surface-variant mb-6">
            Ajoutez des epreuves depuis le catalogue.
          </p>
        </div>
      ) : (
        <div className="flex flex-col lg:flex-row gap-gutter">
          <div className="flex-1 space-y-4">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-surface-container-lowest border border-outline-variant p-6 flex flex-col md:flex-row justify-between gap-4 hover:border-primary transition-colors"
              >
                ...
                <div className="flex items-center gap-3">
                  {item.examPaper.priceWithCorrection && (
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={item.withCorrection}
                        onChange={() =>
                          handleToggleCorrection(item.examPaper.id, item.withCorrection)
                        }
                  ...
                  <button
                    onClick={() => handleRemove(item.examPaper.id)}
          ...
          <div className="lg:w-80 bg-surface-container-lowest border border-outline-variant p-8 h-fit sticky top-28">
            <h3 className="font-headline-sm text-primary mb-6 border-b border-outline-variant pb-4">
              Recapitulatif
            </h3>
